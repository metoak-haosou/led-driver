...
//打开文件函数
static int
open_led2(struct inode *inode, struct file *fp)
{
...
//关闭文件函数
static int
release_led2(struct inode *inode, struct file *fp)
{
...
//读函数
static ssize_t
read_led2 (struct file *fp, char __user *ubuf, size_t count, loff_t *ppos)
{
...
//写函数
static ssize_t
write_led2 (struct file *fp, const char __user *ubuf, size_t count, loff_t *ppos)
{
	printk(KERN_NOTICE"write\n");
	return 0;
}
//文件操作函数集
static struct file_operations fops_led2={
	.owner = THIS_MODULE,
	.open = open_led2,
	.release = release_led2,
	.read = read_led2,
	.write = write_led2,
};
...
led2_init(void)
{
	...
	pMASK_DATA_0_LSW=ioremap(MASK_DATA_0_LSW,2);
	...
	//使能pin9输出
	//iowrite32(0x20000,pDIRM_0);
	printk("the value of pDIRM_0:%x\n",ioread32(DIRM_0));
	//iowrite32(0x20000,pOEN_0);
	printk("the value of pOEN_0:%x\n",ioread32(OEN_0));

	//取反pin9
	//iowrite32(0x20000,pDATA_0);

	printk("the value of pDATA_0:%x\n",ioread32(DATA_0));

	printk("the value of pin0:%x\n",inb(0));
